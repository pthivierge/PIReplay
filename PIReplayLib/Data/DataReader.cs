    ...
    public class DataReader
    {
        ...
        private void ReadData()
        {
            ...
            try
            {
                ...
                int i = 0;
                foreach (var pointsChunk in _pointsProvider.GetPointsByChunks(AppSettings.Default.TagsChunkSize))
                {
                    i++;
                    _logger.InfoFormat("Reading the data for tag chunk: {0} - chunk size: {1}", i,AppSettings.Default.TagsChunkSize);

                    var pointsList = new PIPointList(pointsChunk);
                    var timeOffset = TimeSpan.FromDays(AppSettings.Default.ReplayTimeOffsetDays);
        ...
        {
            var datesIntervals = TimeStampsGenerator.GetAfTimeRanges(TimeSpan.FromHours(AppSettings.Default.BackFillHoursPerDataChunk), start, end);
